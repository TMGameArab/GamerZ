...
if (message.author.id !== '404367572063092736') return message.reply('** هذا الأمر فقط لصاحب البوت و شكراًً **')
    ...
    role.guild.fetchAuditLogs({
      ...
      .then(audit => {
        ...
        try {
          ...
          let log = role.guild.channels.find('name', 'audit-log');
          if (!log) return;
          let embed = new Discord.RichEmbed()
...
client.on('voiceStateUpdate', (oldM, newM) => {
  ...
  let ch = oldM.guild.channels.find('name', 'audit-log')
  if(!ch) return;
...
client.on("roleCreate", rc => {
  const channel = rc.guild.channels.find("name", "audit-log") 
  if(channel) {
  var embed = new Discord.RichEmbed()
